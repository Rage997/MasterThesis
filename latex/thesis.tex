...
We now focus on analysing the Taxonomic Families of the species belonging to the five largest clusters. Out of the 17 Taxonomic Families, only 9 are present in the five largest clusters. This shows that some Taxonomic Families are more keen into co-invasion than others. In Table \ref{table:clusters_species} we reported the percentage of the cluster's species in a taxonomic family if it's present in the cluster. Vascular Plants have a significant relevance. Birds and Insects trajectories in the latent space seems to be in agreement, meaning that they are likely to co-invade a region.

The dimension of the Taxonomic Families is non-homogeneous. As shown in Fig. \ref{fig:hist_tax_fam}, Vascular Plants and the Insects are the largest Taxonomic Families. This should be taken into account when analysing the composition of clusters: a species belonging to one of the most abdundat families will have an higher probability of (indirectly) interacting with other species in the latent space.
...
The clusters for the regions in the latent space are also interesting. For the regions we were able to detect 5 clusters using the OPTICS algorithm and they are reported in Table \ref{table:clusters_region}. 
...
Canada    & Iran, Islamic Republic of & Gibraltar  & Belize              & Niger                    \\ \hline
Estonia   & Mongolia                  & Nepal      & Burkina Faso        & Vietnam                  \\ \hline
Slovakia  & Somalia                   & Suriname   & Libya               & Zambia                   \\ \hline
          &                           &            & Palestine, State of & Central African Republic \\ \hline
          &                           &            & Senegal             &                          \\ \hline
          &                           &            & Tajikistan          &                          \\ \hline
          &                           &            & Gabon               &                          \\ \hline
          &                           &            & Congo               &                          \\ \hline
...
\begin{figure}[ht]
    ...
    \caption{The five clusters of regions on a geo-spatial map.}
    \label{fig:trajectories_region}
\end{figure}



%TODO our findings are in agreement with lady's paper -> comment these findings
...
%2. Analysis done
We showed the applicability of our model it by analysing 15947 alien species invasion given by First Records Dataset. The analysis on the results of our model is based on studying clusters of trajectories in the latent space for both species and regions. However, this approach is limited, in particular for the species. We were able to detect 67 clusters for the species in the latent space, however, we only analysed the largest 5. This approach is acceptable for a first analysis and to show the capabilities of our custom latent space REM, but, for an in-depth analysis this is not sufficient. A more exaustive approach would have been to study the probability of, given a species, or a taxonomic family, find another species inside all the clusters found in the analysis. 
 
%3. limitation: sigma choice is somehow arbitrary
Our approach requires the user to select the amount of noise $\Sigma$ that our model will use in the latent space process. A large $\Sigma$ will grant the model more dynamicity. In contrast, a small $\Sigma$ will make the model more conservative. The choice of this parameter is somehow arbitrary. An user might want to try different values of $\Sigma$ and analyse the dynamics of the latent space. Then, based on this feedback, update the value $\Sigma$ by either increasing or decreasing it. 

The First Records Dataset is currently the most comprehensive and largest dataset available for first time records of alien species. However, it is a biased dataset towards regions such as Europe as a whole and taxonomic groups such as plants and insects. Furthermore, the information about the year of introduction of an alien species in a region is not precise and most of the times does not correspond to the year of actual introduction, which might have happened decades later. These bias and unreliability of the dataset affects our results.

% Recap + conclusion
Concluding, we showed how a relational event model can be applied to a bipartite dynamic network and extended by included a latent space configuration of the nodes. We then presented to the reader a framework to analyse the results based on a clustering algorithm. The improvements and extension of already existing tools such as relational event models into more advanced and efficient methods capable of performing tipe-dependant analysis such as the study of alien species first records will help ecologist to investigate more in depth this ecological phenomena and, with hope, fully understand its dynamics. 
