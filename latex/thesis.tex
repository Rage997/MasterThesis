...
%TODO define theta being the distance of vectors in the latent space
...
By exploiting the natural formulation of our latent space model (Eq. \ref{eq:latentspace}) we can express solve the computationally challenging E-step by means of a Kalman filter. We can express the quantity $Q$ in function of its first two conditioned moments $E$, $V$ TODO add them. By recalling the definitions of a-priori and a-posteriori estimate (Sec. \ref{sec:em}), we write:
...
%TODO linearise the process

However, in contrast to what shown in the background section \ref{sec:kalman}, our process is not linear.
The process .... is not linear. We can consider a linear Gaussian approximation of the latent space formulation 
